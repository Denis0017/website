...
const firmName = "MerchEX International";
...
if (contactForm) {
  contactForm.addEventListener("submit", e => {
    ...
    const comapany = (document.getElementById("comapany") || {}).value?.trim() || "";
    ...
    const to = "merchex.international@gmail.com";
    const subject = encodeURIComponent(`${firmName} - Inquiry from ${name}`);
    const bodyLines = [
      `Name: ${name}`,
      `Company/ Firm: ${comapany}`,
      `Email: ${email}`,
      `Phone: ${phone}`,
      "---",
      `Message: ${message}`
